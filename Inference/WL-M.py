...
parser.add_argument(
    "--root", type=str, help="Path to the directory containing the Adapter checkpoints"
)
parser.add_argument(
    "--save_dir",
    default="/ibex/user/radhaks/LLMs/LLaMA_7B/LLAMA_EMNLP_DeepSpeed/predictions/wers",
    type=str,
    help="directory to save predictions as JSON files",
)
parser.add_argument("--data_path", type=str, help="Path to dataset to run inference on")
parser.add_argument(
    "--pretrained_path", type=str, help="Path to the Alpaca checkpoints"
)
...
def result(adapter_path, model):
    ...
    for datapoint in tqdm.tqdm(data):
        ...
        inf = (
            output[len(tokenizer.decode(encoded)) :].split("\n")[0].strip()
        )  # Removing the prompt from the model output
        ref = ground_truth.strip()

        if (
            inf == ref
        ):  # we increase the count if the inference compleately matches the ground
            c = c + 1
        pr.append(inf)
        gt.append(ref)
    ...
    with open(
        os.path.join(
            save_dir,
            adapter_path.split("/")[-2] + adapter_path.split("/")[-1] + ".json",
        ),
        "w",
    ) as f:
